...
async function run(label, injectCss) {
  ...
  const resultPromise = p.evaluate(() => new Promise((resolve) => {
    ...
    function tick() { const now = performance.now(); frameTimes.push(now - last); last = now;
      if (now < window.__perfStart + 4000) requestAnimationFrame(tick); else resolve(frameTimes); }
    window.__perfStart = performance.now(); requestAnimationFrame(tick);
  }));
  ...
  console.log(`${label}: dropped ${over16}, janky(>33) ${over33}, worst ${worst.toFixed(1)}ms`);
  await ctx.close();
}
await run('current (with content-visibility)', null);
await run('content-visibility disabled', `.section { content-visibility: visible !important; }`);
