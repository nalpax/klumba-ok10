...
// force the plane group to a clearly-visible spot in the open sky (top area, well clear of the lawn oval)
await p.evaluate(() => {
  const g = document.querySelector('.gb-plane');
  if (g) g.style.transform = 'translate(750px, 90px)';
});
await p.waitForTimeout(150);
const box = await p.locator('.garden-backdrop').boundingBox();
await p.screenshot({path:'/tmp/shot8_full_backdrop.png', clip: box});
await p.screenshot({path:'/tmp/shot8_zoom.png', clip:{x: box.x + box.width*0.35, y: box.y, width: box.width*0.35, height: 110}});
